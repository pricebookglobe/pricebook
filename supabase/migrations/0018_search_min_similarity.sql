-- The search function has never excluded weak matches — it only ORDERS
-- by similarity, never filters out a poor one. That let a store with no
-- genuine match for the searched product still surface some other,
-- unrelated item from its own inventory (e.g. searching "Snickers" could
-- return "Cocoa Cream Filled Wafers" from a store that doesn't actually
-- carry Snickers, just because that item scored least-badly among that
-- store's own inventory). The app already has an established "is this a
-- genuinely strong match" threshold (0.75, used elsewhere for the
-- near/city-best callouts and the web-fallback decision) — this applies
-- that same threshold as a hard filter here too, so a weak match is
-- excluded everywhere, not just downstream in some call sites.
