-- 0014 added a HARD filter requiring the stored unit string to exactly
-- match the query's unit string. That was too strict: a merchant storing
-- "liter" while a query parses "L" (or "pcs" vs "piece", "g" vs "gram",
-- etc.) means the same real-world unit but different text, and the hard
-- filter was silently excluding the item entirely — which is almost
-- certainly why a customer standing inside a store that genuinely carries
-- the item still saw "no nearby store." This replaces the hard exclusion
-- with a ranking preference instead: an exact size/unit match sorts
-- first, but a mismatched or unknown one is still returned, never
-- dropped.
